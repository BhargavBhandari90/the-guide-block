...
export default function Edit({ attributes, setAttributes, clientId }) {
	const { steps, blockId, isPreview, showSuccessMessage, successMessage } = attributes;
		...
		<div {...useBlockProps()}>
			<InspectorControls>
				...
				>
					<ToggleControl
						...
						label={ __( 'Show Success Message at the end.', 'how-to-guide-block' ) }
						onChange={() => { setAttributes({ showSuccessMessage: !showSuccessMessage }); }}
					/>
					{showSuccessMessage && (
						<TextControl
							label={ __( 'Success Message', 'how-to-guide-block' ) }
							value={ successMessage }
							onChange={(val) => setAttributes({ successMessage: val })}
							placeholder={ __( 'Enter your success message here', 'how-to-guide-block' ) }
						/>
					)}
				</PanelBody>
			</InspectorControls>
			...
			{isPreview ? (
				...
				<>
					<div className="steps-list">
						...
						{steps.map((step, index) => (
							<div key={index} className="step-card">
								<h4>Step {index + 1}</h4>
								<TextControl
									label={__('Step Title', 'how-to-guide-block')}
									value={step.step}
									onChange={(val) => updateStep(index, 'step', val)}
								/>
								<div className={`step-image-container ${step.imageUrl ? 'has-image' : 'no-image'}`}>
									<MediaUpload
										onSelect={(media) =>
								...
								<RichText
									...
									label={__('Add Step Description', 'how-to-guide-block')}
									value={step.description}
									onChange={(val) => updateStep(index, 'description', val)}
									placeholder={__('Step description', 'how-to-guide-block')}
								/>
					...
					<Button onClick={addStep} variant="primary" style={{ marginTop: 10 }}>
						+ Add Step
					</Button>
